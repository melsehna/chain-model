...
\begin{abstract}
Biofilms expose only their outermost cells to antibiotic, shielding an interior
core from direct killing. Whether this spatial structure aids evolutionary
rescue depends on \emph{how} the core participates in population dynamics.
We analyze a stochastic 1D chain model in which births and deaths drive a
conveyor-belt exchange between a protected core and an antibiotic-exposed edge.
A surprising dormant-core result emerges: if core cells neither grow nor die,
the total mutation supply integrated over the population's decline equals that
of an equivalent well-mixed population---passive protection alone confers no
rescue advantage. The entire biofilm advantage originates from active core
births. Core-born resistant mutants face a tension between two effects:
mutation supply is enriched by the core's lower death rate, but any delivered
mutant enters at the innermost edge position and is immediately at risk of
displacement back into the core before it can expand. The net delivery rate
saturates logarithmically with core size. A single dimensionless parameter,
$\alpha = b_{\mathrm{core}}(N_0 - l)/(s_{\mathrm{edge}}\,l)$, governs which
effect dominates: when $\alpha \ll 1$ (high dose relative to core output) the
core acts as an efficient nursery; when $\alpha \gg 1$ (low dose) displacement
suppresses delivery and the biofilm advantage collapses. Independently, edge
deaths continuously replenish the edge with wild-type core cells, diluting
edge-born resistant lineages during population decline. We validate the
analytical predictions with stochastic simulations across a range of doses,
edge widths, and mutation rates.
\end{abstract}
...
The central result is counter-intuitive: a \emph{dormant} core---one with no
cell division---provides no rescue advantage whatsoever. Shielding cells from
killing is not sufficient; the core must actively produce births to matter.
When it does, the advantage is real but bounded: core-born mutants must
traverse the edge-to-core boundary in reverse, and the displacement risk at
that boundary limits how much the core can contribute. These two effects---
enriched mutation supply from the core, and displacement-driven suppression of
delivery---compete in a way that depends on drug dose, making the biofilm's
advantage strongest at intermediate to high doses and collapsing near the MIC.

The remainder of this paper is organized as follows.
Section~\ref{sec:model} defines the chain model precisely.
Section~\ref{sec:analytics} derives the total mutation supply for biofilm and
well-mixed populations and proves the dormant-core equivalence, then analyzes
establishment probabilities for core- and edge-born mutants and characterizes
the dependence on $\alpha$.
Section~\ref{sec:sims} presents validation against stochastic simulations
across doses, edge widths, and mutation rates.
...
$l = 100$, $\bCore = 0.2$, $\bEdge = 1$) it equals $1 + 0.81 = 1.81$: the biofilm
expects roughly twice as many mutations as the well-mixed control.
