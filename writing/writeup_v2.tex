...
\begin{itemize}
    ...
    $\wR = \bR - \dR = 0.4$ is positive, so once an R lineage
...
$\dR$            & $0.6$         & R death rate in edge ($\wR = 0.4$) \\
...
We sweep dose over $\dEdge \in \{1.05,\,1.1,\,1.2,\,1.4,\,1.6,\,1.9,\,2.3,\,2.8\}$, core activity over $\bCore \in \{0,\,0.05,\,0.1,\,0.2,\,0.4,\,0.8\}$, and edge length $l \in \{0, 25, 50, 100, 200, 400\}$ with $5{,}000$ independent runs at every parameter combination. The well-mixed reference is run at the same set of doses, also $5{,}000$ replicates per dose. Every run terminates naturally on either rescue ($r_{\text{edge}} \geq r_{\text{thr}}$) or extinction ($N = 0$); no artificial time or generation cap is applied. 

During these sweeps, we hold $b_e, b_R, and d_R$ fixed at $1.0, 1.0, and 0.6$ respectively, whereas the swept parameter governing antibiotic dose is represented by $d_e$, which starts at a dose of $1.05$ (just above the WT birth rate $b_E = 1.0$) and goes up to $d_E = 2.8$ (such that at the highest dose, WT experiences 2.8 times more deaths than births in the edge). The core activity parameter $b_c$ is swept from $0$ (fully dormant) to $0.8$ (modeled such that cells in the core experience $b_c \cdot b_e$ activity), with the default value set at $0.2$, where both WT and R cells have the same birth and death rates in the core ($\bCore_WT = \dCore_R$) since we treat it as shielded from antibiotic selection. The edge length $l$ is swept from $0$ (no edge, fully well-mixed) to $400$ (thick edge, thin core), with the default value at $100$[].
...
\subsection{Figure 1 Panel A: dormant core.}
Figure 1 Panel A shows us the result of 5,000 simulations per parameter combination, where we simulate a biofilm and a well-mixed population, such that the biofilm has zero turnover (no births or deaths) in the core, mimicking a completely dormant biofilm. The bar plot demonstrates that when the interior cannot divide, the biofilm rescues less often than the well-mixed population at both low and high antibiotic doses. This suggests that the shielded interior of a biofilm does not buy any benefit when the spatially protected core reservoir is not active. Thus, the "protective" spatial structure of a biofilm could be considered a liability preventing rescue by antibiotic resistance evolution if the core is dormant.

This is likely due to the fact that without any births to generate new mutations in the core, the only source of resistance mutations is from the edge, where growth rates are higher. However, the edge is under much stronger selection pressure from the antibiotic compared to the core which does not experirence antibiotic selection, which may lead to a rapid decline in population size and therefore a lower probability of generating and fixing resistance mutations before extinction occurs. 

In contrast, the well-mixed population has a more uniform exposure to the antibiotic, which may allow for a more gradual decline and a higher chance of resistance mutations arising and taking over before extinction.

\subsection{Figure 1 Panel B: active core.}
Figure 1 Panel B shows us that when the interior is dividing at a steady rate, the result is a flip in rescue probability depending on antibiotic dose (ie., selection pressure). At low dose / low selection pressure, the biofilm has lower probability of rescue compared to well-mixed, just as in Panel A. But at high dose the bars flip: the biofilm rescues roughly twice as often as the well-mixed reference. The same spatial structure that hurts at low dose now helps at high dose. Hence, the "double-edged sword" analogy.

\subsection{Figure 1 Implications}
The biofilm only wins under one specific combination: high dose plus an
active core. Thus, Figure 1 illustrates:
\begin{itemize}
    \item A biofilm with a dormant core never rescues more than well-mixed, no matter the antibiotic dose.
    \item An biofilm with an active core only rescues more than well-mixed at high dose; at low dose it still suppresses antibiotic resistance relative to well-mixed.
\end{itemize}
...
Brief treatment of the establishment penalty. Cite Panel B of
